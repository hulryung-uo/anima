"""drag_to_container must not report success when the lift was rejected.

``drag_to_container`` lifts an item (PickUp 0x07) then drops it into a
container (DropItem 0x08). On a server lift-reject (0x27 LiftRej — item out
of range, locked down, too heavy, or the mobile already holding something)
the item never leaves its slot. The old code fired the DropItem anyway —
against an item it was not holding — and returned ``True`` regardless, so the
caller believed the move happened and stranded the item.
...
from anima.action.interaction import drag_to_container
from anima.perception.world_state import ItemInfo

_CONTAINER = 0x40000015
_CORPSE = 0x12345
_ITEM = 0x9001
...
def test_successful_full_lift_sends_drop_and_returns_true():
...
def test_partial_stack_lift_counts_as_a_real_lift():
    ...
    item = ItemInfo(
        serial=_ITEM, x=10, y=20, z=0, graphic=0x1BF2, amount=50,
        container=_CORPSE,
    )
    ctx = _ctx([item])
    ...
    ok = asyncio.run(drag_to_container(ctx, _ITEM, 20, _CONTAINER))
...
def test_item_not_in_world_still_attempts_the_drop():
    ...
    ok = asyncio.run(drag_to_container(ctx, _ITEM, 1, _CONTAINER))
