...
# ---------------------------------------------------------------------------
# Anti-freeze: procedure timeout, liveness watchdog, starvation breaker
# ---------------------------------------------------------------------------


class _SleepingProcedure(Procedure):
    """Simulates a mutation-broken procedure that blocks forever."""

    def __init__(self, name: str, sleep_s: float = 30.0,
                 timeout_s: float | None = None):
        self.name = name
        self.description = f"Sleeps {sleep_s}s"
    ...
    async def execute(self, ctx):
        ...
        return ProcedureResult(success=True, message="woke up")


class _FailingProcedure(Procedure):
    def __init__(self, name: str):
        self.name = name
        self.description = f"Always fails: {name}"
...
class TestAntiFreeze:
    @pytest.mark.asyncio
    async def test_procedure_timeout(self):
        """A blocked procedure must be cut off by the planner default cap."""
        reg = ProcedureRegistry()
        reg.register(_SleepingProcedure("heal_self", sleep_s=10.0))
        planner = Planner(reg)
        planner._PROC_TIMEOUT_S = 0.05

        ctx = _make_ctx(hits=20, hits_max=100)  # heal_self gets selected
        result = await planner.tick(ctx)
        ...
        # Planner is not wedged — a second tick still runs
        result2 = await planner.tick(ctx)
        assert result2 is not None

    @pytest.mark.asyncio
    async def test_per_procedure_timeout_override(self):
        """A procedure's own timeout_s beats the planner default."""
        reg = ProcedureRegistry()
        reg.register(_SleepingProcedure("heal_self", sleep_s=10.0, timeout_s=0.05))
        planner = Planner(reg)
        planner._PROC_TIMEOUT_S = 60.0
    ...
    async def test_external_cancel_propagates(self):
        """Cancellation NOT from the watchdog (shutdown) must propagate."""
        import asyncio
        ...
        ctx = _make_ctx(hits=20, hits_max=100)
        tick_task = asyncio.create_task(planner.tick(ctx))
        for _ in range(100):
            await asyncio.sleep(0.01)
            if planner._current_proc_task is not None:
                break
        assert planner._current_proc_task is not None
        planner._current_proc_task.cancel()  # external/shutdown cancel
        with pytest.raises(asyncio.CancelledError):
            await tick_task

    @pytest.mark.asyncio
    async def test_starvation_demotes_after_k_failures(self):
        """K consecutive failures temporarily demote the procedure."""
        reg = ProcedureRegistry()
        reg.register(_FailingProcedure("heal_self"))
        planner = Planner(reg)
        ...
        proc = await planner.select_procedure(ctx)
        assert proc is None or proc.name != "heal_self"

    @pytest.mark.asyncio
    async def test_starvation_cooldown_expires(self):
        """Demotion auto-expires after the cooldown."""
        import asyncio
        ...
        reg.register(_FailingProcedure("heal_self"))
        planner = Planner(reg)
        planner._proc_breaker = CircuitBreaker(max_failures=3, cooldown_s=0.05)
        ...
        assert not planner._proc_breaker.is_open("heal_self")

    @pytest.mark.asyncio
    async def test_watchdog_cancels_stalled_task(self):
        """The watchdog cancels a stalled task and arms the fallback window."""
        import asyncio

        reg = ProcedureRegistry()
        planner = Planner(reg)
        planner._WATCHDOG_POLL_S = 0.01
        planner._WATCHDOG_STALL_S = 0.02
        planner._running = True
        planner._last_procedure = "frozen_proc"

        ctx = _make_ctx()  # static position
        stalled = asyncio.create_task(asyncio.sleep(10))
        planner._current_proc_task = stalled

        wd = asyncio.create_task(planner._liveness_watchdog(ctx))
        await asyncio.sleep(0.2)
        planner._running = False
        wd.cancel()

        assert stalled.cancelled() or stalled.done()
        assert planner._watchdog_cancelled is True
        assert planner._force_fallback_until > time.time() - 1
        assert planner._proc_breaker.failure_count("frozen_proc") >= 1
        stalled.cancel()

    @pytest.mark.asyncio
    async def test_fallback_prefers_primary_then_wander(self):
        """Fallback picks registered profession work, else a short walk."""
        reg = ProcedureRegistry()
        reg.register(StubProcedure("chop_wood"))
        planner = Planner(reg)
        ctx = _make_ctx()

        proc = await planner._fallback_procedure(ctx)
        assert proc.name == "chop_wood"

        # Without any profession procedure → random wander walk
        planner2 = Planner(ProcedureRegistry())
        proc2 = await planner2._fallback_procedure(ctx)
        assert proc2 is not None
        assert "anti_freeze_wander" in proc2.name

    @pytest.mark.asyncio
    async def test_forced_fallback_window_in_tick(self):
        """During the fallback window tick() prefers the fallback pick."""
        reg = ProcedureRegistry()
        reg.register(StubProcedure("chop_wood"))
        planner = Planner(reg)
        planner._force_fallback_until = time.time() + 60

        ctx = _make_ctx()
        result = await planner.tick(ctx)

        assert planner._last_procedure == "chop_wood"
